...
func main() {
	...
	recentPosts := <-GetRecentPostsAsync(*client, ctx, "b")
	fmt.Printf("%+v", recentPosts)
...
func GetRecentPostsAsync(client jschan.Client, ctx context.Context, board_name string) <-chan *jschan.GetManageRecentResponse {
	recentPostsChan := make(chan *jschan.GetManageRecentResponse)

	go func() {
		//defer close(recentPostsChan)
